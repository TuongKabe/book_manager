...
export async function getDashboardData(from: Date, to: Date): Promise<DashboardData> {
  ...
  >`
    ...
    FROM (
      SELECT "date"::timestamp AS "date", "totalVnd" AS rev, 0 AS cst, 1 AS oc, 0 AS bs
      FROM "Order"
      WHERE "date" BETWEEN ${from} AND ${to}
      UNION ALL
      SELECT "date"::timestamp AS "date", 0 AS rev, "amountVnd" AS cst, 0 AS oc, 0 AS bs
      FROM "Expense"
      WHERE "date" BETWEEN ${from} AND ${to}
      UNION ALL
      SELECT "soldDate"::timestamp AS "date", 0 AS rev, COALESCE("purchaseCostVnd", 0) AS cst, 0 AS oc, 1 AS bs
      FROM "Book"
